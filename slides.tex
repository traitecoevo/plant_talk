...
\begin{tikzpicture}[remember picture, overlay]%
  ...
  {\normalsize \FA \faBook} \,\,  Falster, FitzJohn, Brännström, Dieckmann, Westoby (2016) {\bf plant: A package for modelling forest trait ecology and evolution}. \emph{Methods in Ecology and Evolution} XX: XXX. doi: \href{http://doi.org/10.1111/2041-210X.12525}{10.1111/2041-210X.12525}\\
    ...
    \vspace{1.5em}
  {\normalsize \FA \faStar} ``Application note'', about new package for R $\rightarrow$ {\plant}
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[anchor = west, text width=12cm, align=left] at ($(current page.west) + (0.25cm, -4cm)$) {
  {\color{white} \Large A simulation model for exploring demographic phenomena}
  };
\end{tikzpicture}
\end{frame}

% NOTES
\begin{frame}{Individual based model $\rightarrow$ key unit is individual plant}
  \begin{center}
...
\begin{frame}{Key methodological advances making {\plant} possible}
  ...
  1990-2000 & Approximation with partial differential equations (PDEs) \\
  ...
  2010-2015 & Higher resolution numerical methods for solving PDEs \\
